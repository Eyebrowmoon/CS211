...
int deletejob_sync(struct job_t *jobs, pid_t pid);
int setjobstop_sync(struct job_t *jobs, pid_t pid);

void print_terminate_msg(pid_t pid, int jid, int signal);
void print_stop_msg(pid_t pid, int jid, int signal);
...
void do_bgfg(char **argv) 
{
    int is_job;         /* Job? or process? */
    int bg = !strcmp(argv[0], "bg");    /* bg? or fg? */
    int jid;            /* Job id */
    pid_t pid;          /* Process id*/
    ...
    is_job = (arg[0] == '%');
    if (is_job) {   /* The argument is jid */
        if (!isDigit(arg + 1)) {    /* Not number jid */
        ...
        if (!job) { /* There is no job with jid */
            printf("%s: No such job\n", arg);
            return;    
        }
    } else {    /* The argument is pid */
        if (!isDigit(arg)) {    /* Not number pid */
            printf("%s: argument must be a PID or %s\n", argv[0], "%jobid");
            return;
        }
        ...
        if (!job) { /* There is no process with pid */
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) { 
        ...
        } else if (WIFSIGNALED(status)) {   /* Abnormal termination by signal */
            jid = deletejob_sync(jobs, pid);
            print_terminate_msg(pid, jid, WTERMSIG(status));
        } else if (WIFSTOPPED(status)) {    /* Stopped by signal */
            jid = setjobstop_sync(jobs, pid);
            print_stop_msg(pid, jid, WSTOPSIG(status));
        } 
    }
...
/* Wrapper of rio_writen which checks the return value */
void Rio_writen(int fd, void *usrbuf, size_t n)
{
...
/* perform deletejob synchronously */
int deletejob_sync(struct job_t *jobs, pid_t pid)
{
    sigset_t mask_all, prev_all;
    int jid;
...
/* Set the state of job to ST synchronously */
int setjobstop_sync(struct job_t *jobs, pid_t pid)
{
    struct job_t *job;
    sigset_t mask_all, prev_all;
...
/* Print [JID] (PID) terminated by signal ... */
void print_terminate_msg(pid_t pid, int jid, int signal)
{
    int length;
    char itoa_buf[12];
...
/* Print [JID] (PID) stopped by signal ... */
void print_stop_msg(pid_t pid, int jid, int signal)
{
    int length;
    char itoa_buf[12];
    ...
    length = itoa_safe(itoa_buf, (int) pid);
    Rio_writen(1, itoa_buf, length);
    Rio_writen(1, ") stopped by signal ", 20);
    length = itoa_safe(itoa_buf, signal);
    Rio_writen(1, itoa_buf, length);
    Rio_writen(1, "\n", 1);
}

/* Check whether all character is digit or not */
int isDigit(char *str) {
    char *strr = str;
